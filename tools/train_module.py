...
def train_sub_module(gpu_id, model_cfg, data_cfg, env_cfg, log_option):
    ...
    if data_cfg['dummy']:
        logger.info('Generate the dummy train/validation dataset.')
        ...
        logger.info('Generate the train/validate dataset.')

        train_pipeline = build_pipeline(data_cfg['pipeline'], mode="train")
        val_pipeline = build_pipeline(data_cfg['pipeline'], mode="validation")
    ...
    for epoch in range(data_cfg['start_epoch'], data_cfg['epochs']):
        ...
        try:
            if validate_mode and epoch % val_freq == 0:
                ...
                is_best.update(validate(val_loader, model, val_params, device,
...
def train(data_loader, model, params, optimizer, epoch, device, train_freq=5):
    ...
    for i, (images, targets) in enumerate(data_loader):
        ...
        try:
            # display
            if i % train_freq == 0:
                display_epoch(epoch, len(data_loader), i+1,
...
def validate(data_loader, model, params, device, best_evaluation, distributed, world_size):
