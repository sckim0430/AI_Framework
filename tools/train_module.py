...
def train_sub_module(gpu_id, model_cfg, data_cfg, env_cfg):
    ...
    if torch.cuda.is_available():
        ...
        if env_cfg['gpu_id'] is not None:
            ...
            print('This is the case with other nodes including the current node with multi gpus in multi processing. You will use {} gpu now.'.format(env_cfg['gpu_id']))
            torch.cuda.set_device(env_cfg['gpu_id'])
            model.cuda(env_cfg['gpu_id'])
            data_cfg.update({'batch_size':int(data_cfg['batch_size']/env_cfg['ngpus_per_node'])})
            env_cfg.update({'workers':int((env_cfg['workers']+env_cfg['ngpus_per_node']-1)/env_cfg['ngpus_per_node'])})
            model = nn.parallel.DistributedDataParallel(model,device_ids=[env_cfg['gpu_id']])
        else:
        ...
        print('This is the case with single node with single gpu in single processing. You will use {} gpu now.'.format(env_cfg['gpu_id']))
        torch.cuda.set_device(env_cfg['gpu_id'])
